...
| Shopping Lists | `/shopping-lists` | `pages/shopping-lists/ShoppingListsPage.tsx` | free | **v2 SHIPPED 2026-05-03**. Transit list (30d TTL via `shopping_list_ttl_sweep` daily, 50-item cap, separate quota from catalog). 3 entry points: manual (name + optional qty/weight/volume), catalog (CatalogAutocomplete), scan (ContextualScannerModal `add_to_shopping_list` action). Per-item price comparison (≤10 entries: brand/store/price/barcode). Buy flow reuses `QuickAddModal` (with new `onSaved` callback); multi-price → `PricePickerDialog`. Cross-page integration: `[+ Add to shopping list]` on catalog item detail. User-side routes at `/api/shopping-lists/*`; admin cross-user GETs unchanged at `/api/admin/shopping-lists/*`. Gated by `shopping_list_v2_enabled` flag. Design + redundancy review at `.claude/docs/pages/shopping-lists.md`. Tier-gated subsections (`checkout_flow`, `trip_notes`, `receipt_scanning`) deferred to follow-up. |
