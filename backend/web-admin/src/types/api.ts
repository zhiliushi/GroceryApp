...
export interface BusinessMetricsSignal {
  kind: 'good' | 'info' | 'warn';
  title: string;
  text: string;
}
...
export interface BusinessMetrics {
  product: { name: string; computed_at: string };
  acquisition: {
    total_users: number;
    new_signups_7d: number;
    new_signups_30d: number;
    by_week: { week: string; signups: number }[];
  };
  activation: {
    activated_users: number;
    activation_rate: number;
    purchase_count_total: number;
    active_purchase_count_total: number;
  };
  engagement: {
    wau: number;
    mau: number;
    wau_mau_ratio: number;
    items_added_7d: number;
    items_resolved_7d: number;
    items_per_wau_7d: number;
  };
  retention: {
    d7: number | null;
    d7_eligible_count: number;
    d7_returned_count: number;
  };
  health: {
    median_today: number | null;
    median_30d_ago: number | null;
    trend: 'up' | 'down' | 'flat';
    sample_size: number;
  };
  revenue: {
    ...
    received_myr: number;
    pct_to_goal: number;
    by_month: { month: string; amount_usd: number }[];
    last_30d_usd: number;
    forecast_months_to_goal: number | null;
    days_since_last_entry: number | null;
    entry_count: number;
  };
  signals: BusinessMetricsSignal[];
  computed_at: string;
}
